        ...
        <div className="flex w-full max-w-sm items-center space-x-2">
            <form
                ...
                <form.Field name="email" children={(field) => {
                    return (<>
                        <Input type="email" name={field.name} value={field.state.value}  onChange={(e) => field.handleChange(e.target.value)} placeholder="Github Email"/>
                    </>)
                }}>
                </form.Field>
                <form.Subscribe
                ...
                >
                    {([canSubmit, isSubmitting]) => (
                        <Button type="submit" disabled={!canSubmit}>
                            {isSubmitting ? "..." : <UserPlus></UserPlus>}
                        </Button>
                    )}
                </form.Subscribe>
            </form>
